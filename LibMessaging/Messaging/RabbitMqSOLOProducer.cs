...
namespace LibMessaging.Messaging;

public interface IRabbitMqSOLOConnectionProvider : IAsyncDisposable
{
    ValueTask<IConnection> GetConnectionAsync(CancellationToken cancellationToken = default);
}

public class RabbitMqSOLOConnectionProvider : IRabbitMqSOLOConnectionProvider
{
    private readonly IConfiguration _config;
    private IConnection? _connection;

    public RabbitMqSOLOConnectionProvider(IConfiguration config)
    {
        _config = config;
    }
    ...
    public async ValueTask<IConnection> GetConnectionAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            Uri = new Uri(_config.GetConnectionString("RabbitMQ") ?? "amqp://guest:guest@localhost:5672/"),
...
public interface IMessageSOLOProducer : IAsyncDisposable
{
    Task PublishAsync<T>(string queueName, T message, CancellationToken cancellationToken = default);
}


public class RabbitMqSOLOProducer : IMessageSOLOProducer
{
    private readonly ILogger<RabbitMqSOLOProducer> _logger;
    private readonly IRabbitMqSOLOConnectionProvider _provider;

    //public IConnection? Connection { get; private set; }
    //public IChannel? Channel { get; private set; }
    private bool _disposed;

    public RabbitMqSOLOProducer(ILogger<RabbitMqSOLOProducer> logger, IRabbitMqSOLOConnectionProvider provider)
    {
        _logger = logger;
        _provider = provider;
    }
